...
export function ArchitectureNodeDetail({ node, relatedLabels }: ArchitectureNodeDetailProps) {
  if (!node) {
    return (
      <div className="rounded-md border border-dashed border-border bg-background/60 p-5">
        <p className="mono-label">Selected node</p>
        <p className="mt-2 text-sm text-muted">
          Select a node to inspect its role in the system.
        </p>
      </div>
    )
  }
    ...
    <article className="rounded-md border border-border bg-surface p-5">
      ...
      {!hasDetails && (
        <p className="mt-3 text-sm text-muted">
          Detailed node information can be added in Sanity Studio.
        </p>
      )}
      ...
      {relatedLabels?.length ? (
        <div className="mt-4">
          <p className="mono-label mb-2">Related</p>
